    ...
    for(c_idx=0; c_idx<3; c_idx++)  {
        ...
        for(class_index = 0; class_index < class && c_idx>=c_idx_min && c_idx<c_idx_max; class_index++)    {
            switch (sao[class_index]->type_idx[c_idx]) {
                case SAO_BAND:
                    sc->hevcdsp.sao_band_filter_wpp[ classes[class_index] ](dst, src, stride,  sao[class_index], edges, width, height, c_idx);
                    
                    break;
                case SAO_EDGE: {
                    sc->hevcdsp.sao_edge_filter_wpp[ classes[class_index] ](dst, src, stride, sao[class_index],  edges, width, height, c_idx);
                    break;
                }
            }
        }
    ...
    for (x = x0 ? x0:8; x < x_end; x += 8) {
        for (y = y0; y < y_end; y += 4) {
            ...
            if (bs) {
                ...
                sc->hevcdsp.hevc_loop_filter_luma[0](src, pixel, sc->frame->linesize[LUMA], no_p, no_q, beta, tc);
    ...
    for (y = y0 ? y0:8; y < y_end; y += 8) {
        for (x = x0 ? x0-8:0; x < x_end; x += 4) {
            ...
            if (bs) {
                ...
                src = &sc->frame->data[LUMA][y * sc->frame->linesize[LUMA] + x];
                sc->hevcdsp.hevc_loop_filter_luma[1](src, sc->frame->linesize[LUMA], pixel, no_p, no_q, beta, tc);
